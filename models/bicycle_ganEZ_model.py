...
class BiCycleGANEZModel(BaseModel):
    @staticmethod
    ...
    def getCZ(self, connecteddArea):
        CZ = torch.from_numpy(np.zeros((5, 8, self.real_B.size(2), self.real_B.size(3)), dtype=np.float32)).to(self.device)
        index = 0
        for c in connecteddArea:
            ...
            if c != -1:
                ...
                if self.is_train():
                    ...
                    matrix = np.zeros((1, self.real_B.size()[1], self.real_B.size()[2], self.real_B.size()[3]), dtype=np.float32)
                ...
                CZ[index: index + 1, :, :, :] = (torch.from_numpy(np.zeros((1, 8), dtype=np.float32)).to(self.device)).view(1, 8, 1, 1).expand(1, 8, self.real_B.size(2), self.real_B.size(3))
            index += 1
    ...
    def fill_z(self, z, zz):
        ...
        # print(z[0][0][0][0])
        newZ = torch.zeros((1, 8, self.real_B.size(2), self.real_B.size(3))).to(self.device)
        for ii in range(8):
            # print(ii)
            # print(z[0][ii][0][0])
            # print(zz[1][ii][0][0])
            left = (z[0][ii][0][0] + zz[1][ii][0][0]) / 2
            bottom = (z[0][ii][0][0] + zz[4][ii][0][0]) / 2
            top = (z[0][ii][0][0] + zz[0][ii][0][0]) / 2
            right = (z[0][ii][0][0] + zz[3][ii][0][0]) / 2
            A = (z[0][ii][0][0] + zz[1][ii][0][0] + zz[0][ii][0][0]) / 3
            B = (z[0][ii][0][0] + zz[3][ii][0][0] + zz[0][ii][0][0]) / 3
            C = (z[0][ii][0][0] + zz[1][ii][0][0] + zz[4][ii][0][0]) / 3
            D = (z[0][ii][0][0] + zz[3][ii][0][0] + zz[4][ii][0][0]) / 3
            # print(left)
            # print(bottom)
            # print(top)
            # print(right)
            ...
            # print(C)
            # print(D)
            newZ[0, ii, 0:25, 0:25] = A.view(1, 1, 1, 1).expand(1, 1, 25, 25)
            newZ[0, ii, 0:25, 25:231] = top.view(1, 1, 1, 1).expand(1, 1, 25, 206)
            newZ[0, ii, 0:25, 231:256] = B.view(1, 1, 1, 1).expand(1, 1, 25, 25)
            newZ[0, ii, 25:231, 0:25] = left.view(1, 1, 1, 1).expand(1, 1, 206, 25)
            newZ[0, ii, 231:256, 0:25] = C.view(1, 1, 1, 1).expand(1, 1, 25, 25)
            newZ[0, ii, 25:231, 231:256] = right.view(1, 1, 1, 1).expand(1, 1, 206, 25)
            newZ[0, ii, 231:256, 231:256] = D.view(1, 1, 1, 1).expand(1, 1, 25, 25)
            newZ[0, ii, 231:256, 25:231] = bottom.view(1, 1, 1, 1).expand(1, 1, 25, 206)
            # print(count)
        return newZ
    ...
    def forward(self):
        ...
        # print(self.z_encoded[0, 0, 20:30, 20:30])
        # print('-----------------')
