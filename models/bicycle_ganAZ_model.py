...
class BiCycleGANAZModel(BaseModel):
    @staticmethod
    ...
    def getCZ(self, connecteddArea):
        CZ = torch.from_numpy(np.zeros((5, 8), dtype=np.float32)).to(self.device)
        index = 0
        for c in connecteddArea:
            ...
            if c != -1:
                ...
                # print(B)
                CZ[index: index + 1, :] = z_encoded
            else:
                CZ[index: index + 1, :] = torch.from_numpy(np.zeros((1, 8), dtype=np.float32)).to(self.device)
            index += 1
    ...
    def fill_z(self, z, zz):
        ...
        # print(z[0][0][0][0])
        newZ = torch.zeros((1, 8)).to(self.device)
        for ii in range(8):
            # print(ii)
            # print(z[0][ii][0][0])
            # print(zz[1][ii][0][0])
            ...
            average =  (z[0][ii] + zz[1][ii] + zz[4][ii] + zz[0][ii] + zz[3][ii]) / 5
            # print(left)
            # print(bottom)
            # print(top)
            # print(right)
