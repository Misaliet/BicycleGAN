...
class BiCycleGANSVZModel(BaseModel):
    @staticmethod
        ...
        for ii in range(opt.nz):
            for i in range(opt.load_size):
                for j in range(opt.load_size):
                    ...
                    # left
                    if j <= i and j <= ri:
                        self.tri2[0][ii][i][j] = 1.0 * (half_size-j)/half_size * (half_size-j)/half_size
                        # count += 1
                    # bottom
                    elif j <= i and j >= ri:
                        self.tri4[0][ii][i][j] = 1.0 * (i-half_size)/half_size * (i-half_size)/half_size
                    # top
                    elif j >= i and j <= ri:
                        self.tri1[0][ii][i][j] = 1.0 * (half_size-i)/half_size * (half_size-i)/half_size
                    # right
                    elif j >= i and j >= ri:
                        self.tri3[0][ii][i][j] = 1.0 * (j-half_size)/half_size * (j-half_size)/half_size
        # print(self.tri1[0:1, 0:1, 118:138, 118:138])
        # print(self.tri2[0:1, 0:1])
        # print(self.tri3[0:1, 0:1])
        # print(self.tri4[0:1, 0:1])
        # print(self.tri1 * 3)
    ...
    def fill_z(self, z, zz):
        ...
        top = self.tri1 * temp[0:1][:][:][:]
        left = self.tri2 * temp[1:2][:][:][:]
        # print(left)
        right = self.tri3 * temp[4:5][:][:][:]
        bottom = self.tri4 * temp[3:4][:][:][:]
        tz = top + left + right + bottom
        tz = z - tz
        # print(tz)
